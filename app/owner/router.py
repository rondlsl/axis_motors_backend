...
async def get_trip_details(
    ...
    **Права доступа**: Только владелец автомобиля
    ...
    route_data = None
    logger.info(f"DEBUG: Car GPS ID: '{car.gps_id}' (type: {type(car.gps_id)})")
    logger.info(f"DEBUG: Car GPS ID is None: {car.gps_id is None}")
    logger.info(f"DEBUG: Car GPS ID is empty string: {car.gps_id == ''}")
    logger.info(f"DEBUG: Trip start_time: {trip.start_time}")
    logger.info(f"DEBUG: Trip end_time: {trip.end_time}")
    logger.info(f"DEBUG: Car info: id={car.id}, name={car.name}, plate={car.plate_number}")
    
    if car.gps_id and trip.start_time and trip.end_time:
        logger.info(f"DEBUG: Attempting to fetch GPS data for device {car.gps_id}")
        try:
            ...
            logger.info(f"DEBUG: GPS data result: {route_data is not None}")
            if route_data:
                logger.info(f"DEBUG: GPS coordinates count: {route_data.total_coordinates}")
        except Exception as e:
            logger.error(f"Ошибка получения GPS данных: {e}")
            import traceback
            logger.error(f"DEBUG: Full traceback: {traceback.format_exc()}")
    else:
        logger.info(f"DEBUG: Skipping GPS fetch - gps_id: {car.gps_id}, start_time: {trip.start_time}, end_time: {trip.end_time}")
