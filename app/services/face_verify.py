...
def verify_user_upload_against_profile(user, upload_file) -> Tuple[bool, str]:
    ...
    ⚠️ ВРЕМЕННО ОТКЛЮЧЕНО - AI проверка закомментирована из-за проблем с памятью
    """
    # ВРЕМЕННО: Пропускаем AI проверку
    print("⚠️ AI Face verification ОТКЛЮЧЕНА (закомментирована)")
    return True, "ok"
